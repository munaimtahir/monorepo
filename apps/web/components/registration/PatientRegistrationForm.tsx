...
export type PatientRegistrationFormProps = {
    /** Link for "Back" in the header (e.g. /operator/worklist or /patients). */
    backHref: string;
    /** Label for the back link (e.g. "Back to worklist", "Back to patients"). */
    backLabel: string;
    /** Link for "Register another patient" after first registration (e.g. /operator/register or /patients/register). */
    registerAnotherHref: string;
    /** When user selects an existing patient from search, navigate to this URL. Default: /patients/{id}. */
    getPatientDetailHref?: (patientId: string) => string;
};

const defaultGetPatientDetailHref = (id: string) => `/patients/${id}`;

export function PatientRegistrationForm({
    backHref,
    backLabel,
    registerAnotherHref,
    getPatientDetailHref = defaultGetPatientDetailHref,
    ...
    const handleSelectPatient = (patient: Patient) => {
        const id = patient.id ?? '';
        if (id) router.push(getPatientDetailHref(id));
    };
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {!registeredPatient ? (
                        ...
                        <div className="space-y-6">
                            ...
                            <Link href={registerAnotherHref} className="inline-block text-sm text-blue-600 underline">
